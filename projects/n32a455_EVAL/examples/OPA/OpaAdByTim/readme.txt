...
1. Function description
    1. TIM1 CH4 triggers ADC1 ADC2 to synchronously inject the voltage data of OPA1 OPA2, TIM8 CH4 triggers ADC3 ADC4 to synchronously inject the voltage data of OPA3 OPA4, and TIM1 TIM8 output is controlled by COMP1 and COMP7 brakes
2. Use environment
...
3. Instructions for use
    System Configuration;
        1. Clock source:
                    HSE=8M,PLL=144M,AHB=144M,APB1=36M,APB2=72M,ADC CLK=144M/10,ADC 1M CLK=HSE/8,OPA CLK=36M,COMP CLK=36M,TIM1 CLK=144M, TIM8 CLK=144M
        2. Interrupt:
                    TIM1 brake interrupt on, steal priority 0, sub priority 0
                    TIM8 brake interrupt on, steal priority 0, sub priority 0
        3. Port configuration:
                    PA5 selected as analog function OPA1 VP
                    PB0 is selected as analog function OPA2 VP
                    PC9 selected as analog function OPA3 VP
                    PC5 selected as analog function OPA4 VP
                    ...
                    PA1 selected as analog function COMP1 INP
                    PA11 is selected as analog function COMP1 OUT
                    PC1 selected as analog function COMP7 INP
                    PC2 is selected as analog function COMP7 OUT
        ...
        4. OPA:
                    OPA1 OPA2 OPA3 OPA4 Voltage follower function, VM has no pins
        5. ADC:
                    ADC1 synchronous injection mode, scan conversion, TIM1 CC4 trigger, 12-bit data right-justified, injection conversion OPA1 output voltage and analog voltage data of internal channel 16
                    ADC2 synchronous injection mode, scan conversion, TIM1 CC4 trigger, 12-bit data right-justified, injection conversion OPA2 output voltage and analog voltage data of internal channel 18
                    ADC1 synchronous injection mode, scan conversion, TIM8 CC4 trigger, 12-bit data right-justified, injection conversion OPA3 output voltage and analog voltage data of internal channel 18
                    ADC2 synchronous injection mode, scan conversion, TIM8 CC4 trigger, 12-bit data right-justified, injection conversion OPA4 output voltage and analog voltage data of internal channel 18
        6. COMP:
                    COMP1 INP selects PA1, INM selects 1.65V, the output triggers TIM1 and TIM8 to brake
                    COMP7 INP selects PC1, INM selects 1.65V, the output triggers TIM1 and TIM8 to brake
        7. TIM:
                    TIM1 6-channel complementary open, brake enable and open interrupt, CH4 trigger output
                    TIM8 6-channel complementary open, brake enable and open interrupt, CH4 trigger output
    Instructions:
        1. Open the debug mode after compiling, and use an oscilloscope or logic analyzer to observe the output waveform of TIM1 and TIM8 and the acquired data read by ADC
